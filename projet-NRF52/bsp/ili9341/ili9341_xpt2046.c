...
bool_e XPT2046_getMedianCoordinates(Sint16 * pX, Sint16 * pY, XPT2046_coordinateMode_e coordinateMode){
	...
	index = 0;
	XPT2046_getCoordinates(&tab[0].x , &tab[0].y, coordinateMode);	//on place le premier point dans la premi�re case du tableau.


	for(n=1; n<NB_POINTS_FOR_MEDIAN; n++)
	{
		if(XPT2046_getCoordinates(&current.x , &current.y, coordinateMode))	//r�cup d'un point
		{
			...
			for(j=(uint8_t)index; j>i; j--)	//d�placement des valeurs plus grandes de 1 case
			{
				tab[j] = tab[j-1];
			}
