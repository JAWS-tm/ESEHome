...
La ligne bidirectionelle de "data" du DHT11 doit �tre tir�e au Vdd par une r�sistance de 4,7k.
Vdd doit �tre entre 3 et 5V.

Etant donn� que le STM32F103 est aliment� en 3,3V, il est recommand� d'alimenter ce capteur en 3,3V.
On peut toutefois aller au del�, mais il faut s'assurer que la broche utilis�e pour le dialogue soit tol�rante 5V en entr�e.
...
#define NB_BITS	42	//les deux bits de poids fort n'appartiennent pas aux donn�es utiles. (il s'agit de la r�ponse du capteur avant la trame utile).
...
static void DHT11_callback_exti(nrfx_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
	...
	if(index < NB_BITS)
	{
		...
		else
		{
			...
			falling_time_us = SYSTICK_get_time_us(); //on conserve la diff�rence entre le front montant et le front descendant
...
running_e DHT11_state_machine_get_datas(uint8_t * humidity_int, uint8_t * humidity_dec, uint8_t * temperature_int, uint8_t * temperature_dec)
{
	...
	switch(state)
	{
		...
		case SEND_START_SIGNAL:
			...
			{
				GPIO_write(DHT11_pin, 1);
				DHT11_set_pin_direction(FALSE);	//configurer pin en entr�e, avec d�tection it externe
