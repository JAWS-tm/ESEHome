...
void SECRETARY_init(void)
{
	...
	nrf_esb_config.selective_auto_ack       = true;	//on rend les acquittements d�pendant de l'argument du transmetteur !...
...
void SECRETARY_frame_parse(nrf_esb_payload_t * payload, msg_source_e msg_source)
{
	...
	if(payload->length > BYTE_POS_DATASIZE && payload->length <= NRF_ESB_MAX_PAYLOAD_LENGTH)
	{
		...
		if(OBJECT_ID == OBJECT_BASE_STATION)
		{
			...
			{
				//le message re�u est pour quelqu'un d'autre
			...
			// Si le destinataire est moi ou qu'il est de ma famille sans uid (000000FF par exemple) � des fins de d�bogage
			if(recipient == RF_DIALOG_get_my_device_id() || recipient == (uint32_t) (OBJECT_ID))
			{
				// On v�rifie que le message provient bien de notre notre base station, si on en a pas on accepte tous les messages
				...
				if(msg_source == MSG_SOURCE_RF)
					SECRETARY_process_msg_to_uart(payload);	//je renvoie le message sur l'UART (pour d�bug)
			}
			else
			{
				//le message est pour quelqu'un d'autre

				//si ce message vient de l'UART (donc d'un d�veloppeur qui test des choses...
				//je le renvoie sur la RF
				if(msg_source == MSG_SOURCE_UART)
					SECRETARY_send_msg(payload->length, payload->data);

				//sinon, je ne fait rien... le message n'�tait pas pour moi.
			}
		}
	}
}
...
//Cette fonction permet de convertir un message re�u sur l'UART en une "fausse trame RF"... � des fins de tests.
